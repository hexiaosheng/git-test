...
Ext.define('ux.NavigationView', {
    ...
    **/
    ...
    onActivate: function (view) {
        if (this.isPopAndPush) {
            this.isPopAndPush = false;
            return;
        }
        ...
        //        console.log(clear);
        if (clear) {
            this.viewStack.splice(config.clear, this.viewStack.length);
            xtype = view.config.xtype || view.getItemId();
            if (xtype != this.viewStack[0]) {
        ...
        this.setClear(clear);
        //设置导航栏
        this.getNavigationBar().updateView(config, this.viewStack);
        //设置其他导航栏
        this.setOtherMenu(config.otherMenu);
    },
    /*项被销毁*/
